...
// Make the query:
$q = "SELECT lname, fname, email, DATE_FORMAT(registration_date, '%M %d, %Y') AS regdat, user_id FROM lmnmigrate ORDER BY registration_date ASC LIMIT $start, $pagerows";
...
//If the page is not the first page then create a Previous link
	if ($current_page != 1) {
		echo '<a href="admin-view-users.php?s=' . ($start - $pagerows) . '&p=' . $pages . '">Previous</a> ';
	}
//Create a Next link
	if ($current_page != $pages) {
		echo '<a href="admin-view-users.php?s=' . ($start + $pagerows) . '&p=' . $pages . '">Next</a> ';
	}
	echo '</p>';
}
include "footer.php";?>
