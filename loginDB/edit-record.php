...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($errors)) {
		...
		if (mysqli_num_rows($result) == 0) {
			...
			$q = "UPDATE logindb SET title='$title', fname='$fn', lname='$ln', email='$e' WHERE user_id=$id LIMIT 1";
			$result = @mysqli_query($dbcon, $q);
...
$q = "SELECT title, fname, lname, email FROM logindb WHERE user_id=$id";
$result = @mysqli_query($dbcon, $q);
if (mysqli_num_rows($result) == 1) {
  ...
  <div class="row">
   <div class="large-12 medium-12 small-12 columns">
	<label>Title
	<input type="text" name="title" size="20" maxlength="20" value="' . $row[0] . '">
   </div>
    <div class="large-6 medium-6 small-12 columns">
      <label>First Name
        <input id="fname" type="text" name="fname" size="30" maxlength="30" placeholder="First Name" value=" ' . $row[1] . '"/>
      </label>
    </div>
     <div class="large-6 medium-6 small-12 columns">
      <label>Last Name
        <input id="lname" type="text" name="lname" size="40" maxlength="40" placeholder="Last Name" value=" ' . $row[2] . '"/>
      </label>
    </div>
  ...
  <div class="row"><!--Beginning of Second Row-->
    <div class="large-12 small-12 columns">
      <label>Email
         <input id="email" type="email" name="email" size="50" maxlength="50" placeholder="Email" value=" ' . $row[3] . '"/>
      </label>
    </div>
      <div class="large-12 small-12 columns">
