...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($errors)) {
		...
		$q = "SELECT user_id FROM finalpost WHERE(email='$e' AND psword=SHA1('$p'))";
		...
		if ($num == 1) {
			...
			$q = "UPDATE finalpost SET psword=sha1('$np') WHERE user_id=$row[0]";
			$result = @mysqli_query($dbcon, $q);
...
<form action="register-password.php" method="post">
	...
	<div class="large-6 medium-6 small-12 columns">
		<label>Email
			<input id="email" type="email" name="email" size="40" maxlength="60"placeholder="Email" value="<?php if (isset($_POST['email'])) {
	echo $_POST['email'];
}
?>"/>
		</label>
...
<div class="large-6 medium-6 small-12 columns">
	<label>New Password
		<input id="psword1" type="password" name="psword1" size="12" maxlength="12" placeholder="Confirm Password" value="<?php if (isset($_POST['psword1'])) {
	echo $_POST['psword1'];
}
?>"/>
	</label>
