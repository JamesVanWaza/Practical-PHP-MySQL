...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($errors)) {
		...
		$q = "SELECT user_id FROM postaldb WHERE email = '$e' AND user_id != $id";
		$result = @mysqli_query($dbcon, $q);
		if (mysqli_num_rows($result) == 0) {
//The email address is not already registered or it
			//belongs to the user being updated, therefore, update the postaldb table
			$q = "UPDATE postaldb SET fname='$fn', lname='$ln', email='$e', class='$class', paid='$paid'
...
$q = "SELECT fname, lname, email, class, paid
FROM postaldb WHERE user_id=$id";
$result = @mysqli_query($dbcon, $q);
